...
@api_router.get("/automation/rules/{property_id}")
...
@api_router.post("/automation/run/{property_id}")
async def run_automation(property_id: str, current_user: dict = Depends(require_roles("admin", "manager"))):
    """Manually trigger automation check for a property — finds matching bookings and sends messages"""
    ...
    for rule in rules:
        ...
        if trigger == "pre_arrival":
            # Bookings checking in tomorrow (or within timing window)
        ...
        for booking in bookings:
            ...
            # Check if already sent for this rule + booking
            ...
            try:
                if channel == "email" and guest_email:
                    ...
                    # These will be sent when real credentials are configured
                    # For now, log as "queued" since sandbox mode
                    settings = await db.channel_settings.find_one({"property_id": property_id}, {"_id": 0})
...
@api_router.get("/automation/logs/{property_id}")
...
@api_router.get("/automation/stats/{property_id}")
...
@api_router.post("/automation/preview")
