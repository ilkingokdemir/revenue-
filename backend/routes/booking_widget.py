...
def create_booking_widget_router(db, require_roles):
    ...
    async def widget_info(property_id: str):
        ...
        if not rooms:
            rooms = [
                {"id": "standard", "name": "Standard Room", "base_rate": 100, "max_occupancy": 2, "description": "Comfortable room with all essentials"},
                {"id": "deluxe", "name": "Deluxe Room", "base_rate": 150, "max_occupancy": 2, "description": "Spacious room with premium amenities"},
                {"id": "suite", "name": "Suite", "base_rate": 250, "max_occupancy": 4, "description": "Luxurious suite with separate living area"},
            ]
        ...
        return {
            ...
            "rooms": [{"id": r.get("id", ""), "name": r.get("name", ""), "base_rate": r.get("base_rate", 0), "max_occupancy": r.get("max_occupancy", 2), "description": r.get("description", ""), "amenities": r.get("amenities", [])} for r in rooms],
