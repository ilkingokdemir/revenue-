...
const TESTIMONIALS = [
  {
    ...
    name: "Selin Aydın", role: "Revenue Manager", hotel: "Aurora Palace İstanbul · 142 rooms",
  },
  {
    ...
    name: "Marco Bianchi", role: "Director of Revenue", hotel: "Harborline Suites · 86 rooms",
  },
  {
...
function ForecastMockup() {
  return (
    <div className="relative" data-testid="rq-hero-mockup">
      <div className="absolute -inset-6 bg-[#1D4ED8]/15 rounded-[32px] rotate-2" aria-hidden="true" />
      <div className="relative bg-[#111827] rounded-2xl border border-white/10 shadow-[0_24px_80px_rgba(0,0,0,0.5)] overflow-hidden">
        ...
        <div className="p-5 space-y-4">
          <div className="grid grid-cols-3 gap-3">
            {[["RevPAR", "€160", "+12.4%"], ["Forecast occ.", "91%", "next 30d"], ["Repriced today", "42", "rates"]].map(([l, v, d]) => (
          ...
          <div className="bg-white/[0.04] border border-white/10 rounded-xl p-4">
            <div className="flex items-center justify-between mb-2">
              ...
              <span className="text-[10px] text-blue-300 font-bold flex items-center gap-1"><Sparkles size={10} /> 94% accuracy</span>
            </div>
            <svg viewBox="0 0 320 110" className="w-full h-28">
              ...
              <circle cx="240" cy="44" r="4" fill="#3B82F6" />
            </svg>
            <div className="flex items-center gap-4 text-[9px] text-stone-400 font-bold uppercase tracking-widest">
              <span className="flex items-center gap-1.5"><span className="w-3 h-0.5 bg-blue-500 inline-block rounded" /> AI forecast</span>
...
const FEATURES = [
  { icon: LineChart, title: "Demand forecasting", desc: "24-month forecast with confidence bands, version control and approval workflow. Know your busy dates before the market does." },
  { icon: Gauge, title: "AI dynamic pricing", desc: "Prices recalculated continuously with explainable logic — every change tells you why. Advisory mode or full autopilot with guardrails." },
  { icon: Zap, title: "Intraday repricing", desc: "Hourly pickup-spike detection reprices within the day, not just overnight. Catch the surge while it's still worth money." },
  { icon: Radar, title: "Competitor rate radar", desc: "24/7 compset scanning with under/over-priced alerts and target rates versus the market median." },
  { icon: Target, title: "Restriction advisor", desc: "AI-suggested MLOS and closed-to-arrival restrictions that protect your shoulder nights around peak dates." },
  { icon: CalendarRange, title: "Gap-filler campaigns", desc: "Low-occupancy windows detected automatically, promo codes generated, campaigns drafted — with ROI tracked per campaign." },
  { icon: SearchX, title: "Lost demand tracking", desc: "Denials and regrets logged to reveal unconstrained demand — price to what the market wanted, not just what you sold." },
  { icon: Sunrise, title: "Morning brief", desc: "Your 8 AM heads-up: pickup, parity issues, overnight AI actions and the dates that need a human eye. Coffee-length reading." },
];

const STEPS = [
  { n: "01", title: "Connect your data", desc: "Native with MyHotelBox — one click. Other PMS via API during onboarding. Historical bookings, rates and compset flow in automatically." },
  { n: "02", title: "The AI learns your market", desc: "Forecast models train on your seasonality, pickup curves and competitor behaviour. You review the first suggestions in advisory mode." },
  { n: "03", title: "Approve — or let it fly", desc: "Set guardrails (floor, ceiling, max daily change) and switch on autopilot. Every decision stays explainable and reversible." },
];
    ...
    <div className="min-h-screen bg-[#0A0F1C] text-white antialiased" data-testid="reveniq-page">
      ...
      <header className="sticky top-0 z-50 backdrop-blur-xl bg-[#0A0F1C]/80 border-b border-white/10">
        <div className="max-w-7xl mx-auto px-5 sm:px-8 h-16 flex items-center justify-between">
          <a href="/reveniq" className="flex items-center gap-2.5" data-testid="rq-logo">
            ...
            <span className="font-extrabold tracking-tight text-lg">Reveni<span className="text-blue-400">Q</span></span>
          </a>
          ...
          <div className="hidden md:flex items-center gap-3">
            <button onClick={goLogin} data-testid="rq-signin-btn"
              className="px-4 py-2 rounded-lg text-sm font-bold text-stone-300 border border-white/20 hover:border-white/50 transition-colors">Sign In</button>
        ...
        {mobileNav && (
          <div className="md:hidden border-t border-white/10 bg-[#0A0F1C] px-5 py-4 space-y-3" data-testid="rq-mobile-menu">
            ...
            <a href="/" className="block text-sm font-bold text-blue-400">MyHotelBox ↗</a>
      ...
      <section className="max-w-7xl mx-auto px-5 sm:px-8 pt-16 pb-20 lg:pt-24 lg:pb-28 grid lg:grid-cols-12 gap-12 items-center">
        <motion.div className="lg:col-span-6" initial={{ opacity: 0, y: 28 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, ease: [0.22, 1, 0.36, 1] }}>
          <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-[#1D4ED8]/15 border border-[#1D4ED8]/40 text-blue-300 text-xs font-bold mb-6" data-testid="rq-hero-badge">
          ...
          <h1 className="text-5xl lg:text-6xl font-extrabold tracking-tighter leading-[1.05]">
            ...
            <span className="text-blue-400">that never sleeps.</span>
          </h1>
          ...
          <div className="mt-8 flex flex-wrap items-center gap-4">
            ...
            <button onClick={() => scrollTo("#features")} data-testid="rq-hero-explore-btn"
              className="px-7 py-3.5 rounded-lg border border-white/25 hover:border-white/60 text-white font-bold transition-colors">
              See the engines
            </button>
          </div>
          <div className="mt-10 flex flex-wrap gap-x-8 gap-y-3">
            {[["+12.4%", "avg. RevPAR lift"], ["94%", "forecast accuracy"], ["38k", "price decisions / week"]].map(([v, l]) => (
      ...
      {/* Features */}
      <section id="features" className="border-t border-white/10 bg-[#0D1424] py-24">
        <div className="max-w-7xl mx-auto px-5 sm:px-8">
          <motion.div {...fadeUp} className="max-w-2xl mb-14">
            <div className="text-xs font-bold uppercase tracking-[0.2em] text-blue-400 mb-3">The engines</div>
            <h2 className="text-3xl lg:text-4xl font-bold tracking-tight">Fifteen automation engines. One quiet revenue team.</h2>
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-5">
            {FEATURES.map((f, i) => (
              <motion.div key={f.title} {...fadeUp} transition={{ ...fadeUp.transition, delay: (i % 4) * 0.06 }}
                className="bg-white/[0.04] border border-white/10 rounded-2xl p-6 hover:-translate-y-1 hover:border-[#1D4ED8]/50 transition-[transform,border-color] duration-300"
                data-testid={`rq-feature-${i}`}>
                <div className="w-10 h-10 rounded-lg bg-[#1D4ED8]/15 border border-[#1D4ED8]/30 flex items-center justify-center mb-4">
      ...
      <section id="how" className="max-w-7xl mx-auto px-5 sm:px-8 py-24">
        <motion.div {...fadeUp} className="max-w-2xl mb-14">
          <div className="text-xs font-bold uppercase tracking-[0.2em] text-blue-400 mb-3">How it works</div>
        ...
        <div className="grid md:grid-cols-3 gap-6">
          {STEPS.map((s, i) => (
            <motion.div key={s.n} {...fadeUp} transition={{ ...fadeUp.transition, delay: i * 0.08 }}
              ...
              <div className="text-5xl font-black text-[#1D4ED8]/30 leading-none font-mono">{s.n}</div>
              <h3 className="mt-4 text-lg font-semibold">{s.title}</h3>
              <p className="mt-2 text-sm text-stone-400 leading-relaxed">{s.desc}</p>
            </motion.div>
          ))}
        </div>
        <motion.div {...fadeUp} className="mt-10 flex items-center gap-3 bg-white/[0.04] border border-white/10 rounded-2xl px-6 py-5">
          <Building2 size={18} className="text-blue-400 shrink-0" />
          <p className="text-sm text-stone-300">
            Running your operations on <a href="/" className="font-bold text-blue-400 hover:text-blue-300 transition-colors" data-testid="rq-crosssell-mhb-link">MyHotelBox</a>? ReveniQ shares the same data — it switches on in one click, no integration project.
          </p>
        </motion.div>
      </section>
      ...
      <section className="border-y border-white/10 bg-[#0D1424] py-24">
        <div className="max-w-7xl mx-auto px-5 sm:px-8">
          <motion.div {...fadeUp} className="max-w-2xl mb-14">
            <div className="text-xs font-bold uppercase tracking-[0.2em] text-blue-400 mb-3">From revenue teams</div>
          ...
          <div className="grid md:grid-cols-3 gap-6">
            {TESTIMONIALS.map((t, i) => (
              <motion.div key={t.name} {...fadeUp} transition={{ ...fadeUp.transition, delay: i * 0.08 }}
                ...
                <div className="text-4xl font-black text-blue-500/30 leading-none">"</div>
      ...
      <section id="pricing" className="max-w-7xl mx-auto px-5 sm:px-8 py-24">
        <motion.div {...fadeUp} className="max-w-2xl mb-14">
          <div className="text-xs font-bold uppercase tracking-[0.2em] text-blue-400 mb-3">Pricing</div>
          <h2 className="text-3xl lg:text-4xl font-bold tracking-tight">Pays for itself. Usually in the first month.</h2>
        ...
        <div className="grid md:grid-cols-3 gap-6 items-start">
          {[
            {
              name: "Essentials", price: "€3", note: "per room / month",
            ...
            {
              name: "Pro", price: "€5", note: "per room / month",
            ...
            <motion.div key={p.name} {...fadeUp} transition={{ ...fadeUp.transition, delay: i * 0.08 }}
              className={`rounded-2xl p-7 border transition-transform duration-300 hover:-translate-y-1 ${p.highlight ? "bg-[#1D4ED8] border-blue-400/40 shadow-[0_24px_60px_rgba(29,78,216,0.35)] md:-mt-4" : "bg-white/[0.04] border-white/10"}`}
              ...
              <div className="mt-3 flex items-baseline gap-2">
                ...
                <span className={`text-xs font-medium ${p.highlight ? "text-blue-200" : "text-stone-400"}`}>{p.note}</span>
              </div>
              <p className={`mt-2 text-sm ${p.highlight ? "text-blue-100" : "text-stone-400"}`}>{p.desc}</p>
              <ul className="mt-6 space-y-2.5">
                {p.items.map((it) => (
                  <li key={it} className={`flex items-start gap-2.5 text-sm ${p.highlight ? "text-white" : "text-stone-300"}`}>
                    <CheckCircle2 size={15} className={`mt-0.5 shrink-0 ${p.highlight ? "text-white" : "text-emerald-400"}`} /> {it}
                  </li>
                ))}
              </ul>
              <button onClick={() => scrollTo("#demo")} data-testid={`rq-pricing-cta-${p.name.toLowerCase()}`}
                className={`mt-7 w-full py-3 rounded-lg text-sm font-bold transition-colors ${p.highlight ? "bg-white text-[#1D4ED8] hover:bg-blue-50" : "border border-white/25 hover:border-white/60 text-white"}`}>
                {p.cta}
              </button>
            </motion.div>
          ))}
      ...
      {/* Demo form */}
      <section id="demo" className="border-t border-white/10 bg-[#0D1424]">
        <div className="max-w-7xl mx-auto px-5 sm:px-8 py-24 grid lg:grid-cols-12 gap-12">
          <motion.div {...fadeUp} className="lg:col-span-5">
            <div className="text-xs font-bold uppercase tracking-[0.2em] text-blue-400 mb-3">Get started</div>
            <h2 className="text-3xl lg:text-4xl font-bold tracking-tight">See ReveniQ price your hotel</h2>
      ...
      <section id="faq" className="max-w-4xl mx-auto px-5 sm:px-8 py-24">
        <motion.div {...fadeUp}>
          <div className="text-xs font-bold uppercase tracking-[0.2em] text-blue-400 mb-3">FAQ</div>
        ...
        <div className="max-w-7xl mx-auto px-5 sm:px-8 py-20">
          <div className="grid lg:grid-cols-12 gap-12">
            <div className="lg:col-span-7">
              <h2 className="text-4xl lg:text-5xl font-extrabold tracking-tighter leading-tight">Your rates should<br />work the night shift.</h2>
            ...
            <div className="lg:col-span-5 grid grid-cols-2 gap-8 text-sm">
              <div>
                ...
                <ul className="space-y-2.5 text-[#9CA3AF]">
                  ...
                  <li><a href="/" className="hover:text-white transition-colors font-bold text-blue-400" data-testid="rq-footer-mhb-link">MyHotelBox — Hotel PMS ↗</a></li>
                </ul>
              </div>
              <div>
          ...
          <div className="mt-16 pt-8 border-t border-white/10 flex flex-wrap items-center justify-between gap-4 text-xs text-[#9CA3AF]">
            ...
            <span className="flex items-center gap-2"><ShieldCheck size={13} /> Explainable AI · Guardrails always on</span>
          </div>
        </div>
      </footer>
    </div>
