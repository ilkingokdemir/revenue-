...
async def run_id_verification(db, booking: dict, img: str, by: str) -> dict:
    """AI ile kimlik okuma + rezervasyon ismiyle eşleştirme (paylaşılan çekirdek)."""
    ...
    try:
        ...
        chat = LlmChat(
            api_key=api_key,
            session_id=f"idv-{booking['id'][:8]}-{datetime.now(timezone.utc).timestamp():.0f}",
            system_message=(
...
def create_id_verification_router(db):
    ...
    async def verify(body: VerifyIn,
        ...
        return await run_id_verification(db, booking, img, current_user.get("email", ""))
