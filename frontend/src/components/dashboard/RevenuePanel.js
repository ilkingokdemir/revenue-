...
import { BarChart3, CalendarDays, Settings2, Zap, CheckSquare, Users, Search, Wand2 } from "lucide-react";
...
const RateCalendarTab = ({ propertyId }) => {
  ...
  for (const d of cal.days) {
    ...
    const dayIdx = (offset + d.day - 1) % 7;
    if (!weeks[weekIdx]) weeks[weekIdx] = Array(7).fill(null);
    weeks[weekIdx][dayIdx] = d;
  }
      ...
      <div className="bg-white border border-stone-200 rounded-2xl overflow-hidden">
        ...
        {weeks.map((w, wi) => (
          <div key={wi} className="grid grid-cols-7 border-b last:border-0">
            {w.map((d, di) => (
              <div key={di} className={`border-r last:border-0 min-h-[90px] p-2 ${d?.is_today ? "bg-violet-50 ring-2 ring-violet-300 ring-inset" : d ? "bg-white" : "bg-stone-50"}`}
...
export const RevenuePanel = ({ properties, activePropertyId }) => {
  ...
  useEffect(() => {
    axios.get(`${API}/revenue/pricing-strategy-full/${pid}`).then(r => {
      setRoomTypes(r.data.room_types || []);
    }).catch(() => {});
  }, [pid]);
      ...
      <div className="flex items-center gap-1 mb-6 overflow-x-auto pb-1 border-b border-stone-200">
        {TABS.map(t => (
          <button key={t.id} onClick={() => setTab(t.id)}
            className={`flex items-center gap-1.5 px-4 py-2.5 text-sm font-medium whitespace-nowrap transition-all border-b-2 -mb-[1px] ${
              tab === t.id ? "text-violet-700 border-violet-500 bg-violet-50/50" : "text-stone-400 border-transparent hover:text-stone-600"
            }`} data-testid={`rev-tab-${t.id}`}>
