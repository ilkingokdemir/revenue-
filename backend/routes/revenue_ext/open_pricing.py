...
def create_open_pricing_router(db, require_roles):
    ...
    async def optimize(body: dict,
        ...
        """Open Pricing Optimizer: her segment×kanal×tarih hücresi için BAĞIMSIZ fiyat üretir.
        suggested = baz fiyat × segment faktörü × kanal net-eşitleme faktörü × talep faktörü.
        apply=true → önceki optimizer override'ları silinip yenileri yazılır."""
        ...
        days = max(3, min(int(body.get("days", 14) or 14), 30))
        ...
        for ch in [c["id"] for c in CHANNELS]:
            ...
            # komisyonun %60'ı kanala yansıtılır (net eşitleme); direct'e %3 avantaj
        ...
        for i in range(days):
            ...
            demand = round(0.92 + occ * 0.33, 4)  # 0.92 (boş) → 1.25 (dolu)
            cells = {}
                ...
                for ch, cf in ch_factor.items():
                    ...
                    if apply:
                        await db.open_pricing_overrides.insert_one({
                            ...
                            "reason": "optimizer", "scope_label": f"{seg} × {ch}",
                            "is_active": True,
