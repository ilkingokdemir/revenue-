...
export function InlinePayment({ t, booking, amountMode, fmt, onPaid, onFallback, createIntent, confirmIntent, compact = false, title, payLabel }) {
  const { t: tr, lang } = useLanguage();
  const mountId = useRef(`mhb-pe-${Math.random().toString(36).slice(2, 9)}`);
  const [state, setState] = useState("loading");
  const [err, setErr] = useState("");
  const [amount, setAmount] = useState(0);
  const stripeRef = useRef(null); const elementsRef = useRef(null); const mounted = useRef(false); const piRef = useRef("");

  useEffect(() => {
    let live = true;
    (async () => {
      try {
        const { data } = createIntent ? await createIntent() : await axios.post(`${API}/payments/intent`, { booking_id: booking.id, amount_mode: amountMode });
        const Stripe = await loadStripeJs();
        if (!live) return;
        setAmount(data.amount); piRef.current = String(data.client_secret || "").split("_secret")[0];
  ...
  const pay = async () => {
    ...
    try {
      const { data } = confirmIntent ? await confirmIntent(piRef.current) : await axios.post(`${API}/payments/intent/confirm`, { booking_id: booking.id });
      if (data.paid) onPaid(data); else { setErr(tr("inline.pending")); setState("ready"); }
    } catch { setErr(tr("inline.pending")); setState("ready"); }
  };

  return (
    <div className={compact ? "w-full" : "max-w-2xl mx-auto px-4 sm:px-6 lg:px-8 py-10"} data-testid="inline-payment-step">
      <div className={`bg-white border border-gray-200 ${compact ? "p-4" : "p-6"}`} style={{ borderRadius: t.borderRadius }}>
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-lg font-semibold text-slate-900 flex items-center gap-2" style={{ fontFamily: t.fonts.heading }}><CreditCard size={20} style={{ color: t.colors.accent }} /> {title || tr("inline.title")}</h2>
        ...
        <div id={mountId.current} data-testid="inline-payment-element" />
        ...
        {state === "error" ? (
          ...
          <button onClick={pay} disabled={state !== "ready"} className="mt-4 w-full py-3.5 rounded-lg font-bold text-white text-sm flex items-center justify-center gap-2 disabled:opacity-50" style={{ background: t.colors.accent, borderRadius: t.borderRadius }} data-testid="inline-pay-btn">
            <Lock size={16} weight="fill" /> {state === "paying" ? tr("processing.title") : payLabel ? payLabel(amount ? fmt(amount) : "") : tr("payment.payAndComplete", { amount: amount ? fmt(amount).replace(/^[^\d]*/, "") : "" })}
          </button>
        )}
