...
async def startup_event():
    ...
    # iter 367 — Scheduled Check-out tick worker (every 5 min)
    async def _scheduled_checkout_tick():
        import asyncio as _asyncio
        while True:
            try:
            ...
            await _asyncio.sleep(300)  # every 5 min
    import asyncio
    asyncio.create_task(_scheduled_checkout_tick())

    # iter 368 — Scheduled Reports tick worker (every 5 min)
    async def _reports_tick():
    ...
    asyncio.create_task(_reports_tick())
