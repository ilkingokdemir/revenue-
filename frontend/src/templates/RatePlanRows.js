...
export function RatePlanRows({ t, room, plans, nights, cart, onAdd }) {
  ...
  return (
    <div className="border-t border-gray-100 divide-y divide-gray-100" data-testid={`rate-plans-${room.id}`}>
      {list.map((p, idx) => {
        const nightly = planNightPrice(room.base_price, p);
        const q = qty[p.id] || 1;
        const total = nightly * nights * q;
        const inCart = cart?.find((c) => c.room.id === room.id && (c.plan?.id || "") === (p.id || ""));
        const firstNightly = planNightPrice(room.base_price, list[0]);
        ...
        return (
          <div key={p.id || idx} className="grid grid-cols-1 md:grid-cols-[1.4fr_1fr_auto] gap-3 md:gap-4 p-4 items-center transition-colors hover:bg-slate-50/60"
            ...
            <div className="md:text-right">
              ...
              <div className="text-2xl font-bold text-slate-900" data-testid={`plan-total-${room.id}-${p.code || idx}`}>£{total.toFixed(0)}</div>
              <div className="text-[11px] text-slate-500">£{nightly.toFixed(0)} {tr("room.perNight")} · {tr("room.includesTaxes")}</div>
            </div>
            <div className="flex items-center gap-2 md:justify-end">
              ...
              <button type="button" onClick={() => onAdd(room, p.id ? p : null, q)} disabled={!room.is_available}
