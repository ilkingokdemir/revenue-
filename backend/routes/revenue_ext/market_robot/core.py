...
BÖLÜM HARİTASI (create_market_robot_router içi, yaklaşık satırlar):
  ~40   _scrape_booking_date / fiyat ayarlama çekirdeği
  ~370  config + neighborhood + geo tarama uçları
  ~1130 rekabetçi fiyatlama + haftalık özet
  ~1550 _do_scan + arz verisi + loglar
  ~1980 doluluk/pickup/talep panoları + aksiyon akışı
  ~2510 performans raporu (gider/gelir forecast builder'ları)
  ~3120 rakip CRUD + keşif + auto-heal + fleet operasyonları
  ~4860 vision zenginleştirme + toplu rakip ekleme
  ~5340 rakip tarama + pulse + gap kapama + AI fleet optimize
  ~6280 otomatik fiyat/etkinlik/rakip tarama yardımcıları
  ~6760 tarayıcı kontrol + oda sayısı + YoY + manuel metrikler
  ~7930 sıralama analizi + market pulse + auto_scan_loop
"""
