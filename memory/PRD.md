...
## Güncelleme (Faz 34 — Google Bağlantı Sihirbazı + Personel Ödül + Kök Neden Görevi + Konu Bazlı Rakip Kıyası) — iteration_613 backend 11/11, frontend %100
- GBP OAuth (gbp_publish.py): GET /gbp/oauth/config, /gbp/oauth/start?property_id (state → db.oauth_states; GOOGLE_CLIENT_ID/SECRET yoksa 400 + redirect URI), GET /gbp/oauth/callback (code→refresh token, TOKEN_ENCRYPTION_KEY varsa Fernet ile şifreli, db.google_tokens{property_id}), GET /gbp/oauth/locations (accountmanagement v1 + businessinformation v1), POST /gbp/oauth/select-location, DELETE /gbp/oauth/disconnect. gbp_publish_reply(): PUT mybusiness v4 .../reviews/{r}/reply (4096 bayt). reviews.approve → platform google & bağlı ise gerçek yayın (events google_published / google_publish_failed). /gbp/status connection + client_configured + redirect_uri. UI: ReviewAgentPanel GbpConnectCard (gbp-connect-btn, konum seç, bağlantıyı kes; ?gbp=connected|error toast). Redirect URI: {PUBLIC_BASE_URL}/api/gbp/oauth/callback. GEREKLİ ENV (kullanıcıdan): GOOGLE_CLIENT_ID, GOOGLE_CLIENT_SECRET, (opsiyonel) TOKEN_ENCRYPTION_KEY; Google projesi Business Profile API onaylı olmalı.
- Personel Ödül: reviews.run_staff_praise_weekly_internal → 7g staff_intelligence top_praised → admin e-posta (MOCK) + WhatsApp (MOCK) + db.staff eşleşirse çalışana; db.notifications staff_praise; db.staff_praise_log. POST /reviews/staff-praise/run, GET /reviews/staff-praise/log. JOB 'staff_praise_weekly' (09:00, sadece Pazartesi çalışır). UI: AnalyticsPanel staff-praise-run-btn.
- Kök Neden Görevi: morning_karne.build_karne → root_cause high+recurring konu → staff_tasks {source root_cause, topic, department (TOPIC_DEPT), frequency_pct_at_open} 7 günde 1; çözülen (done/resolved/completed/closed) görevler 'Kök neden görevleri (7g)' satırında "%açılış → %şimdi".
- Konu Kıyası: GET /reputation/topic-compare/{pid}?days (8 konu: cleanliness, staff, breakfast, check-in, location, value, noise, wifi; bizim = sentiment topics/anahtar kelime ort. puan; rakip = Places API reviews varsa live, yoksa SIMULATED deterministik). UI: AnalyticsPanel Competitors sekmesi topic-compare-card.
- MOCK: Resend, Twilio, Places/GBP (anahtar yok).

## Güncelleme (Faz 35 — Rakip Yorum Zekâsı + Ödül Duvarı + Görev Kapanış Etkisi) — iteration_614 backend 6/6, frontend %100
- Rakip Zekâsı: GET /reputation/competitor-intel/{pid}?days — rakip yorumları (Places API varsa live, yoksa SIMULATED seed) → TOPIC_KW/AMENITY_KW anahtar kelime ile güçlü/zayıf/övülen olanaklar; 'they_have_we_dont' = rakiplerde övülen olanak ∩ (bizim yorumlar + properties.amenities'te yok); their_strength_our_weakness. UI: AnalyticsPanel Competitors → competitor-intel-card.
- Ödül Duvarı: GET /reviews/staff-praise/leaderboard (tüm roller) → weekly_stars (staff_praise_log, hafta başına 1), badges (⭐/🥈2×/🥇3×/🏆5×), monthly_leaderboard (staff_intelligence 30g). UI: MyTasksPanel reward-wall (rozetler + aylık liderlik).
...
## Güncelleme (Faz 38 — Muhasebe Konektörleri + Kuyruk Paneli + Tenant/RBAC/SSO) — iteration_617 backend 16/16 (fix sonrası), frontend %100
- routes/finance_ext/accounting_sync.py: Xero (ManualJournals, xero-tenant-id, Idempotency-Key) + QBO (JournalEntry, realmId, QBO_SANDBOX=true) OAuth2 start/callback/disconnect; build_daily_journal (DR tahsilat / CR net gelir / CR vergi / AR clearing; payments.date + bookings.check_out); push_journal idem_key {pid|prov|date|daily}; token yoksa status 'mock' (outbox). e-Fatura: build_ubl_tr (UBL-TR 1.2 TEMELFATURA) → db.efatura (status draft; EFATURA_INTEGRATOR_KEY varsa sent), bookings.efatura_no. Endpoints /accounting/connectors/{pid}, /settings, /mapping/{prov}/{pid}, /journal/preview|run|list, /efatura/issue|list|xml. JOB accounting_daily_sync 03:30. ENV: XERO_CLIENT_ID/SECRET, QBO_CLIENT_ID/SECRET, EFATURA_INTEGRATOR_KEY. UI: AccountingConnectorsCard (AccountingExportPanel üstü).
- JobQueueCard (SystemHealthPanel üstü): sayaçlar, filtre, yeniden dene.
- routes/security/tenant_sso.py: organizations {allowed_domains, sso{provider, google_hd, ms_tid, default_role, auto_provision}}; /orgs CRUD, /orgs/{id}/assign (property_ids/user_ids; users _id veya id), /orgs/users/{id}/property-roles; users.property_roles {pid: role}; admin.has_permission(..., property_id) tesise özel rolü uygular (reviews approve/publish kullanır). OIDC: /auth/sso/providers, /auth/sso/{google|microsoft}/start (PKCE+nonce+state, hd) /callback (JWKS ile id_token doğrulama, org eşleşmesi, auto-provision, access/refresh cookie). ENV: GOOGLE_SSO_CLIENT_ID/SECRET, MS_SSO_CLIENT_ID/SECRET, MS_SSO_TENANT_ID. UI: LoginPage SsoButtons (yapılandırılmamışsa disabled), AdminPanel OrganizationsCard.
- KALAN (sıradaki): Tek Dil Arayüz (401 panel i18n'e taşıma — büyük iş, kademeli), sertifika rehberi (e). Kullanıcıdan bekleniyor: Google GBP + SSO + Xero/QBO + e-Fatura entegratör anahtarları.

## Güncelleme (Faz 39 — Sertifika Rehberi + i18n (Review Ops) + Hesap Eşleme + Tesis Rol Editörü) — iteration_618 backend %100, frontend %85 → i18n seçici düzeltildi (screenshot doğrulandı)
- routes/platform_ext/certification_guide.py: GET /certifications/guide — 8 madde (booking, expedia, whatsapp, resend, stripe, gbp, sso, accounting) adımlar + env + otomatik done (env/db). UI: AdminPanel 'Canlıya Geçiş' sekmesi (CertificationGuideTab).
- i18n: tr.json/en.json 'ro.*' anahtarları; ApprovalQueuePanel, ReviewOpsBits, AnalyticsPanel review-ops kartları useTranslation('@/i18n'). Sidebar'a 'dashboard' varyantlı LanguageSelector (lang-tr/lang-en/lang-de, mhb_lang). FAB pill (maint_lang) artık global setLang'i de tetikler. NOT: iki ayrı i18n sistemi var — src/i18n/index.js (dashboard, useTranslation) ve src/i18n/LanguageContext.js (booking widget, useLanguage); LanguageSelector variant='dashboard' index.js'i kullanır, diğer varyantlar LanguageContext ister. Kalan ~390 panel hâlâ hard-coded (kademeli taşıma backlog).
- AccountingConnectorsCard MappingEditor (acct-map-{prov}-{payments|revenue|tax|ar}, acct-map-save-*) → POST /accounting/mapping/{prov}/{pid}.
- AdminPanel PropertyRoleEditor (prop-roles-btn-{uid} popover: tesis + rol ekle/kaldır, etkin rol önizleme) → PUT /orgs/users/{id}/property-roles.

## Güncelleme (Faz 40 — Booking Engine Dönüşüm Paketi / Paket 1) — iteration_619 backend 6/6, frontend %100
- Kullanıcı isteği: Booking engine + web sitesi şablonlarını Mews/Cloudbeds/eviivo ile kıyasla, tüm eksikleri (12 madde) sırayla kapat. YENİ engine YAZILMADI; mevcut BookingEngine.js + 10 platform şablonu genişletildi.
- routes/pms/be_conversion.py: GET /booking/price-calendar/{pid}?month&adults (gün bazlı min fiyat, müsaitlik, sold_out, is_cheapest sadece fiyat farkı varsa), GET /booking/rate-plans/{pid} (public, yoksa 3 varsayılan plan seed: flexible 0%, non_refundable -10% deposit 100, breakfast +£15/gece), admin CRUD /be-rate-plans/{pid}[/{id}], GET /booking/flex-dates/{pid}?check_in&check_out&adults&span (±3 gün alternatif + saving_pct), POST /booking/reserve-multi {items:[{room_type_id, rate_plan_id, qty}], promo_code, gift_card_code, source} → her satır ayrı booking, ortak cart_ref; master booking cart_master/cart_total/cart_items/cart_refs. payments.booking-checkout amount = cart_total; ödeme sonrası cart kardeşleri de paid.
- UI: templates/PriceCalendar.js (2 aylık grid, tıkla giriş→çıkış, en düşük fiyat yeşil, dolu üstü çizili, son odalar noktası), SearchWidget 'price-calendar-toggle', templates/RatePlanRows.js (oda altında plan satırları yan yana, qty +/-, Seç/Eklendi), templates/CartBar.js (yapışkan sepet + Devam Et), RoomCards FlexDatesStrip (flex-date-{offset}), GuestDetailsStep BookingSummary sepet satırları + 'Başka oda ekle', ConfirmationStep confirm-cart-items. Booking engine'e Türkçe (tr) dil bloğu eklendi (translations.js). Admin: BookingEngineV2Panel 'Fiyat Planları' sekmesi (RatePlansCard).

## Güncelleme (Faz 41 — Web Sitesi Paketi + Dağıtım Paketi / Paket 2-3) — iteration_620 backend 27/27, frontend %100
- site_builder.py: 6 tema (classic, modern, boutique, coastal, urban, nature), content.blocks (10 blok sıralı/enabled), content.faqs, content.pages_enabled, map_query; public_site artık oda fotoğrafı/açıklama, reviews (4★+), rating{avg,count}; POST /site-builder/public/contact → db.site_inquiries + notification; admin GET/PUT /site-builder/{pid}/inquiries.
- UI: HotelSitePage.js çok sayfalı (/site/{pid}/{home|rooms|gallery|location|faq|contact}, pushState), site/siteThemes.js, site/SiteBlocks.js (AvailabilityBlock canlı müsaitlik → /book?check_in&check_out&adults, RoomsBlock, ReviewsBlock, MapBlock (Google Maps embed, anahtar gerekmez), FaqBlock (FAQPage JSON-LD), ContactBlock, GalleryBlock), skip-link + CookieBanner. SiteBuilderExtras.js: BlocksEditor (sıra/aç-kapa/sayfalar), FaqEditor, InquiriesCard. Booking engine URL'den check_in/check_out/adults okuyup otomatik arar.
- routes/pms/be_distribution.py: Google Hotel Ads — GET /hotel-ads/hotel-list/{pid}.xml, GET /hotel-ads/ari/{pid}.xml?days&nights (Transaction/Result, plan bazlı Baserate+Refundable), GET /hotel-ads/landing (302 → /book, utm_source=google_hotel_ads, site_visits source), admin GET /hotel-ads/status/{pid} (6 kontrol: geo lat/lon eksikse uyarı, feed URL'leri, POS şablonu, adımlar). Embed: GET /embed/widget.js?property&accent&radius&lang&target (CORS *), admin GET /embed/snippet/{pid}. Hediye çeki: GET /booking/gift-cards/config/{pid}, PUT /gift-cards/config/{pid}, POST /booking/gift-cards/purchase (Stripe Checkout, tx type gift_card; STRIPE yoksa MOCK aktif), GET /booking/gift-cards/status/{card_id}?session_id, POST /booking/gift-cards/check; reserve-multi gift_card_code → bakiye düşer, redemption_log. Exit-intent: GET /booking/exit-intent/{pid}, PUT/GET /exit-intent/config/{pid} (promo_codes otomatik upsert), POST .../shown. payments.booking-checkout amount_mode=deposit → plan deposit_pct toplamı, deposit_due/balance_due/payment_plan.
- UI: templates/GiftCardSection.js (landing'de satış + ?gift=success modal), templates/ExitIntentPopup.js (mouseout/visibilitychange, sessionStorage, kodu uygula → pendingPromo), CookieBanner (localStorage mhb_cookie_consent), GuestDetailsStep hediye çeki kullan + 'Şimdi £X depozito öde' seçeneği + Visa/Mastercard/Apple Pay/Google Pay rozetleri (Stripe Checkout cüzdanları Stripe panelinden açılır), reserve-multi source=utm_source. Admin: BookingEngineV2Panel 'Dağıtım' sekmesi (DistributionCard: Hotel Ads kontrol listesi + feed kopyala, embed snippet, exit-intent ayarı, hediye çeki ayarı).
- NOT: Google Hotel Center hesabı ve tesis latitude/longitude kullanıcı tarafından girilmeli. Apple/Google Pay Stripe Checkout'ta Stripe Dashboard → Payment methods'tan açılır.
- Kalan backlog: Yetki Matrisi, Yevmiye Takvimi, TR e-Fatura entegratör anahtarı, Housekeeping offline mobil.

## Güncelleme (Faz 42 — Yetki Matrisi, Yevmiye Takvimi, Tesis Konumu, Hediye Çeki E-postası) — iteration_621 backend 20/20, frontend %95
- routes/security/permission_matrix.py: GET /admin/permission-matrix?property_id (kullanıcı × tesis × modül etkin izin; öncelik custom_permissions > tesis rolü > genel rol), GET /admin/permission-matrix.csv (indirilebilir), GET /admin/permission-changes; log_perm_change() hook'ları: tenant_sso property-roles PUT, admin users/{id}/permissions PUT, auth_routes users/{id} role PUT. DEFAULT_PERMISSIONS'a viewer/staff/housekeeper/maintenance eklendi (runtime ile matris tutarlı). UI: AdminPanel 'Yetki Matrisi' sekmesi (PermissionMatrixCard: filtre, arama, geçmiş, CSV).
- Tesis konumu: GET/PUT /properties/{pid}/location (lat/lon/adres/ülke kodu/telefon), POST /properties/{pid}/geocode (OpenStreetMap Nominatim, anahtar gerekmez); PropertyUpdate modeline latitude/longitude. UI: DistributionCard → Hotel Ads kartı içinde PropertyLocationCard (adresten bul, OSM önizleme, kaydet → geo kontrolü yeşil). aldgate-flats: 51.5142,-0.0755 kaydedildi → Hotel Ads 'Hazır'.
- Yevmiye takvimi: GET /accounting/journal/calendar/{pid}?month (gün: pushed/mock/failed/missing/future, sağlayıcı bazlı detay, resend_candidates), POST /accounting/journal/resend/{pid} {dates:[..]} (failed→superseded, run_daily_sync_internal). UI: AccountingExportPanel → JournalCalendarCard (ay gezinme, lejant sayaçları, gün detayı, tek gün / toplu yeniden gönder).
- Hediye çeki e-postası (mevcut gift_cards.py genişletildi): gift_card_html() markalı şablon (template_settings accent), send_gift_card_emails() alıcı + satın alana kopya, idempotent (email_sent_at); admin issue'da otomatik gönderim, POST /gift-cards/{id}/resend-email; Stripe ödeme onayında (payments.py gift_card tx + be_distribution status/mock) otomatik gönderim. UI: GiftCardsPanel 'E-posta gönder / ↻' butonu. E-posta Resend anahtarı yokken MOCK (outbox).

## Güncelleme (Faz 43 — Finans kısayolu, Yevmiye uyarısı, Yetki denetim PDF, Hediye çeki tasarımları) — iteration_622 backend 16/16 (design fix sonrası), frontend %100
- Finans menüsüne `finance-accounting` (core) → AccountingExportPanel + JournalCalendarCard (Raporlar altındaki accounting-export ile aynı panel).
- Yevmiye uyarısı: accounting_sync.run_journal_alert_check (başarısız → anında; 2 gün eksik → uyarı; e-posta (liste boşsa tüm admin/manager) + WhatsApp (_send_whatsapp_reply) + notifications; idempotent journal_alerts), GET/PUT /accounting/journal/alerts/config/{pid}, POST /accounting/journal/alerts/run/{pid}?force; workers.journal_alert_loop (6 saat). UI: JournalCalendarCard 'Uyarılar' paneli.
- Yetki denetim PDF: GET /admin/permission-changes/report.pdf?month&property_id (reportlab, DejaVu font, değişiklik günlüğü + dönem sonu etkin roller). UI: PermissionMatrixCard ay seçici + 'Denetim PDF'.
- Hediye çeki tasarımları: gift_cards.GIFT_DESIGNS (classic, festive, summer, spring), gift_card_html(design), GET /gift-cards/designs, GET /gift-cards/preview (HTML), issue/resend design alanı, gift_card_config.design + designs_enabled, public config designs listesi, public purchase design (izinli değilse varsayılana düşer). UI: DistributionCard tasarım ızgarası + iframe önizleme; GiftCardsPanel tasarım seçimi + önizleme; booking engine GiftCardSection tasarım çipleri (kart görseli canlı değişir).

## Güncelleme (Faz 44 — BE & Web Sitesi kalan 11 eksik) — iteration_623 backend 16/17 (1 skip), frontend doğrulandı (brand accent + EN çeviri screenshot ile teyit)
- TEK FİYAT MOTORU: /booking/rooms artık widget_pricing.nightly_rates (RMS override + owner override), check_los_restrictions (los_block → is_available false) ve explain_price döndürür (nightly[], stay_total, avg_nightly, price_explanation, vat_rate, city_tax_per_night, extra_bed_price, child_policy). be_conversion._overrides tüm rate_overrides kaynaklarını okur. reserve-multi: LOS 400, extra_beds, children_ages (child_policy free_under_age / child_price_per_night), KDV ayrıştırma + şehir vergisi (tax_breakdown), üye indirimi (loyalty_email → loyalty_members/guest_profiles tier: bronze5/silver8/gold10/platinum15), manage_url (/guest-portal-v2?ref&email). Yeni: POST /booking/member-rate, GET/PUT /booking/pricing-settings/{pid}.
- UI (/book): RatePlanRows roomNightBase(avg_nightly), fmt (useCurrency.format) tüm fiyatlarda (plan, sepet, özet, onay), price-explain satırı, los-block, ekstra yatak seçici, üye fiyatı barı (MemberRateBar) + member-rate satırı, RoomDetailModal (foto galeri, olanaklar, gecelik fiyatlar, açıklama), özet: KDV dahil satırı + şehir vergisi + üye indirimi, onayda 'Rezervasyonumu yönet' linki. Analytics hook (GA4/GTM/Pixel) + begin_checkout/purchase olayları.
- Web sitesi: content.translations {en,de}, posts (blog/campaign, slug), analytics {ga4_id,gtm_id,pixel_id}, brand {accent,radius}; SITE_PAGES + 'blog'; public endpoints /site-builder/public/analytics/{pid}, /sitemap/{pid}.xml (hreflang), /robots/{pid}.txt; contact honeypot ('website') + IP rate limit 5/saat (429). HotelSitePage: ?lang=tr|en|de seçici, hreflang link'leri, PAGE_LABELS/UI sözlükleri, blog listesi + detay (/blog/{slug}), oda detay sayfası (/rooms/{id}), brand accent/radius override, sitemap link. Admin: TranslationsEditor, PostsEditor, AnalyticsBrandEditor.
- YAPILMADI: Stripe Payment Element (sayfa içi ödeme) — platform Stripe playbook'u sadece Checkout (yönlendirme) akışını destekliyor; Apple/Google Pay Checkout'ta zaten mevcut.

## Güncelleme (Faz 45 — Sayfa içi ödeme, çocuk yaşı, kampanya kuponu, AI çeviri) — iteration_624 backend 8/8, frontend %95
- routes/pms/be_payments.py: GET /payments/config (publishable key, inline_enabled), POST /payments/intent (stripe PaymentIntent, automatic_payment_methods → kart/Apple Pay/Google Pay; idempotent stripe_pi_id; amount_mode deposit), POST /payments/intent/confirm (sunucu doğrulaması → booking + cart kardeşleri paid, payment_transactions, onay e-postası). `stripe` pip paketi eklendi (requirements.txt). UI: templates/InlinePayment.js (Stripe.js CDN, Payment Element tabs, tema rengi, TR/EN/DE locale, hata → 'Güvenli ödeme sayfasına geç' fallback = eski Checkout). BookingEngine: kart/depozito seçildiğinde STEPS.PAYMENT'ta sayfa içi ödeme.
- Çocuk yaşı: SearchWidget misafir seçicide child-age-{i} select'leri; reserve-multi items[0].children_ages; özetle 'Çocuk ücreti' satırı (child_policy free_under_age / child_price_per_night).
- Kampanya kuponu: posts.promo_code + discount_pct → promo_codes otomatik upsert (source campaign_post); blog detayında kupon kodu + CTA `/book?property&promo=KOD`; BookingEngine ?promo= → pendingPromo → detay adımında otomatik uygulanır.
- AI çeviri: POST /site-builder/{pid}/translate {lang, headline, about, seo_*, faqs} (Emergent LLM gpt-5.2, JSON) → TranslationsEditor '✨ AI ile çevir' (kaynak boşsa pasif); translations[lg].faqs desteklendi.
- Bilinen UX notu: Dashboard şube açılır menüsünden tesis seçimi Site Builder'a bazen yansımıyor (genel activePropertyId kablolaması) — bu tur kapsam dışı.

## Güncelleme (Faz 46 — BE Eksikleri Paket 2: Fiyat Garantisi, Day-Use Takvimi, Wishlist Alarmı, Zincir Harita) — iteration_632 backend 15/15, frontend %95
- be_gaps.py: POST /booking/brg-claim (public, e-posta+http url+sayı doğrulama, 24s SLA), GET /booking/brg-claims/{pid}, PUT /booking/brg-claims/{id} {approved|rejected, matched_price, note} → booking brg_matched_price + MOCK e-posta. GET /booking/day-use-availability/{pid}?date (day_use kotası gece stoğundan ayrı, be-settings day_use_pct/start/end). POST/GET /booking/wishlist owner_email + price_at_save; workers._wishlist_alerts (≥%5 ucuzlama veya ≤2 oda → günde 1 MOCK e-posta). GET /booking/chain-search geo (Nominatim fallback) + best (base_price 0 odalar 'best' olamaz).
- UI: ChainSearchPage (/book-all, /search-all) Leaflet artık yerel npm paketi (`leaflet` 1.9.4, unpkg ORB sorunu çözüldü); aynı koordinattaki tesisler tek pinde gruplanır ('N tesis · GBP X'dan'), popup tüm tesisleri listeler. SearchWidget day-use-toggle/day-use-slots; ConfirmationStep brg-section formu; BookingEngine wishlist paylaşımında e-posta istemi (alarm için). Admin: BookingEngineV2Panel 'Fiyat Garantisi' sekmesi → BrgClaimsCard (onayla/reddet, bekleyen sayacı; tesis 'all' ise uyarı).
- NOT: Birçok Londra tesisi şehir merkezi fallback koordinatını paylaşıyor → tesis konumu (Dağıtım → Tesis Konumu) girildiğinde ayrı pinlere ayrılır.

## Güncelleme (Faz 47 — Boşluk Denetimi: Public API v1, OPERA Cloud konnektörü, Harita Filtreleri) — iteration_633 backend 23/23, frontend %100
- KULLANICI KURALI: Mevcut modül/özellik varsa YENİDEN YAPMA, sadece eksiği yap. Denetim sonucu: Otonom robotlar zaten mevcut (AI Pricing auto-apply + rollback, intraday reprice, occupancy_rule_loop, price_guard_loop, profit_autopilot stop-sell, OTA stop-sell forecaster, arrival_reminder T-2, precheckin_reminder_loop, review autopilot, ai/agents.py) → yeni robot yazılmadı. Bağlantı kataloğu + PMS Connect mevcuttu → sadece OPERA Cloud eklendi. Public API iskeleti (p0_pack hbx_ anahtar, GET/POST bookings, rates, guests) mevcuttu → eksikler tamamlandı.
- routes/distribution/public_api_v1.py: api_key_auth (scope kontrolü, anahtar başına rate_per_min sliding bucket, X-RateLimit-Limit/Remaining/Reset, 429 + Retry-After, public_api_calls logu), GET /public/v1/availability, GET /public/v1/rate-plans, GET /public/v1/bookings/{ref|id|external_ref}, POST /public/v1/bookings/{ref}/cancel (webhook booking.cancelled), GET /public/v1/docs (endpoint tablosu + curl örnekleri, x-forwarded-host), PUT /public-keys/{pid}/{key_id} (active/scopes/rate_per_min/name), GET /public-keys/{pid}/usage. p0_pack: anahtar üretiminde scopes+rate_per_min, POST /public/v1/bookings → create_booking_from_api (room_type doğrulama 422, müsaitlik 409, Idempotency-Key → 200 aynı kayıt, 201 yeni, booking_ref MHB-, MOCK onay e-postası, booking_created event), GET bookings status/check_in_from filtreleri. server.py rate_limit_mw: endpoint kendi X-RateLimit başlığını yazmışsa ezmez.
- pms_connect.py: 'opera-cloud' (OHIP) PROVIDERS/_REQUIRED/_translate (par/v1 pricingSchedules)/test-connection (oauth/v1/tokens) — MOCK iskelet. connector_catalog: PROVIDERS içindeki tüm sağlayıcılar için connected/ready + last_test_ok; opera-cloud panel=pms-connect. UI: ConnectorCatalogPanel 'Bağlan' → localStorage mhb_pms_connect_provider → PmsConnectHub sağlayıcıyı otomatik seçer.
- UI: PublicApiKeysCard (Süper Admin panelinde; ad, istek/dk, scope çipleri, üret, iptal/aktifleştir, 7g kullanım, Dokümantasyon & curl). ChainSearchPage FilterBar: maks. fiyat kaydırıcı, yıldız çipleri, sadece müsait, sayaç, sıfırla; harita/liste anlık.

## Güncelleme (Faz 48 — Webhook Retry, API Anahtar Süresi, Harita Rota Bilgisi, Konnektör Senkron Günlüğü) — iteration_634 backend 11/11, frontend %100
- routes/platform_ext/partner_ops.py: emit_webhook (p0_pack'ten taşındı, geriye dönük import korunur) → teslimat kaydı status delivered/retrying/failed, attempts, next_retry_at (1→5→30→120→720 dk, 6 deneme), payload saklanır; retry_due_webhooks + webhook_retry_loop (60 sn); retry_delivery_now; check_api_key_expiry (7 gün içinde bitecek/biten anahtar → notifications + MOCK e-posta admin/manager'a, anahtar başına 1 kez; expiry_warned/expired_notified bayrakları) + api_key_expiry_loop (6 saat). server.py'de spawn edildi.
- public_api_v1.py: _expiry(expires_in_days|expires_at) / _days_left; PUT /public-keys/{pid}/{key_id} expires_at/expires_in_days/extend_days; POST /public-keys/{pid}/expiry-check; GET /webhook-subs/{pid}/deliveries?status; POST /webhook-subs/{pid}/deliveries/{id}/retry; PUT/DELETE /webhook-subs/{pid}/{sub_id}; POST /webhook-subs/{pid}/test (test.ping). p0_pack: anahtar create expires_at, list days_left/expired; webhook-subs list counts + secret maskeli.
- pms_connect.py: _push canlı hata → pms_push_log result.ok=false/error (önce loglanmıyordu); GET /{provider}/sync-timeline/{pid}?days (health healthy/degraded/no_data, gün bazlı ok/failed/mocked, last_success_at, last_error, failed_entries); POST /{provider}/resync/{pid}/{log_id} (aynı standard_rows ile yeniden push, resynced_at).
- be_gaps.py chain_search: _geo_meta (şehir merkezi Nominatim → db.geo_cache; en yakın istasyon Nominatim bounded viewbox ±0.02°; properties.geo_meta sig önbelleği) → centre_km, station{name,km}. Overpass sandbox'tan erişilemiyor → Nominatim kullanıldı.
- UI: WebhookSubsCard (Süper Admin; abonelik ekle/duraklat/sil/test, sayaçlar, teslimat tablosu + 'Şimdi dene'), PublicApiKeysCard geçerlilik seçici + rozet (⚠ N gün / Süresi doldu) + '+90 gün', PmsSyncTimeline (PmsConnectHub'da sağlayıcı seçilince; sağlık, barlar, son hata, başarısız kayıtlar → 'Yeniden senkronla', 'Son push'u yeniden gönder'), log satırında ✖ hata / ↻ yeniden gönderildi; ChainSearchPage satır + popup'ta 'Merkeze X km · 🚉 İstasyon Y km'.

## Güncelleme (Faz 49 — Webhook HMAC İmzası, Anahtar Kullanım Grafiği, Yürüme/Havalimanı, Senkron Uyarı E-postası) — iteration_635 backend 23/23, frontend doğrulandı
- partner_ops.py: serialize_payload (sort_keys, tek serileştirme) + sign_payload (X-Webhook-Signature v1=HMAC-SHA256("<ts>.<body>")) + X-Webhook-Timestamp; verify_signature (5 dk tolerans, compare_digest). public_api_v1: POST /webhook-subs/{pid}/{sub_id}/rotate-secret, POST /webhook-subs/{pid}/verify-signature (partner test aracı), DOCS.webhook_signature (python/node snippet). UI: WebhookSubsCard 'Doğrulama örneği' + 'Secret yenile'.
- Kullanım grafiği: GET /public-keys/{pid}/usage?days=30 → dates, series{key_id:[{date,calls}]}, anomalies{key_id:{today,avg_7d,factor}} (bugün ≥20 ve 7g ort ×3). UI: PublicApiKeysCard '30g grafik' bar chart + '⚠ olağandışı trafik ×N' rozeti.
- be_gaps chain_search geo: Nominatim sandbox'tan 429 verdiği için Photon (komoot) reverse geocoding'e geçildi (_photon, _nearest osm_tag railway:station r=3km / aeroway:aerodrome r=80km, _is_airport çok dilli tercih, _city_centre Photon→Nominatim yedeği, geo_cache). geo_meta v7: centre_km, centre_walk_min, station{name,km,walk_min}, airport{name,km}; eksik arama → sig kaydedilmez + retry_after 1 saat. properties.latitude/longitude varsa geo olarak kullanılır (Tesis Konumu ile uyum). UI: '📍 Merkez ~N dk yürüme' (≤3 km) / km, '🚉 … ~N dk', '✈ … km' (liste + popup).
- pms_connect.py: check_sync_alert (ardışık N canlı hata → notifications + MOCK e-posta admin/manager veya config.emails; aynı seri için 1 kez; başarılı canlı push resolved_at yazar), pms_sync_alert_config {enabled, threshold(2-20), emails}; GET /sync-alerts/{pid}?provider, PUT /sync-alerts/config/{pid}, POST /sync-alerts/{pid}/simulate?provider (admin test), GET /sync-alerts-open/all. UI: PmsSyncTimeline kırmızı banner + ayar satırı (checkbox/eşik); PmsConnectHub üstünde tüm tesisler için açık kesinti şeridi (şube 'all' iken de görünür).
- NOT: Backend hot-reload bazen takılıyor (WatchFiles Reloading… sonrası süreç başlamıyor) → `sudo supervisorctl restart backend` (~40 sn).

## Güncelleme (Faz 50 — Booking Engine Rakip Analizi Paket 39: 8 eksik) — iteration_636/637 backend 25/25, frontend %95
- Analiz: Mews/Cloudbeds/SiteMinder/Profitroom/D-EDGE kıyası → 8 eksik (kullanıcı: hepsi). Mevcut BE yeniden yazılmadı.
- (1) Dönüşüm izleme: site/analytics.js yeniden yazıldı — GA4 e-ticaret (view_item_list, view_item, add_to_cart, begin_checkout, add_payment_info, purchase tek sefer trackPurchaseOnce), Meta Pixel standart olay eşlemesi, Google Ads conversion (site-builder analytics gads_id/gads_label; SiteBuilderExtras inputları), dataLayer ecommerce push, çerez onayı (essential → tag yok; CookieBanner 'mhb-consent' event → sonradan yükler). BookingEngine.js finishRef ile inline ödeme sonrası purchase.
- (2) Huni: routes/pms/be_extras.py POST /booking/funnel/event (public, session_id/device/utm; be_funnel_events), GET /booking/funnel/{pid}?days (adım/drop/cihaz/kaynak/günlük/biggest_drop + öneri). trackFunnel (sessionStorage sid + utm). UI BookingEngineV2Panel 'Huni Analitiği' (BeFunnelCard).
- (3) Ödeme planı: routes/pms/be_payment_plans.py — compute_deposit (plan deposit_pct), be_payments create_intent depozito → Stripe Customer + setup_future_usage=off_session; confirm → schedule_balance (deposit_paid, balance_due, balance_due_date = check_in − balance_charge_days_before, payment_schedule); balance_charge_loop 6 saat (hatırlatma balance_reminder_days_before + off-session tahsilat; başarısızsa misafire öde-linki + admin bildirimi); GET /booking/payment-schedule/{ref}?email, POST pay-now/confirm; admin GET /booking/payment-schedules/{pid}, PUT settings, POST run, POST {booking_id}/charge. Split: POST /booking/{ref}/split (eşit paylar, MOCK e-posta linkleri, 409 tekrar), GET /booking/split/{token}, POST intent/confirm, admin mark-paid-test → partial/paid. Sayfalar /pay-balance/{ref}?email, /pay-share/{token} (pages/PayPage.js, LanguageProvider sarmalı, InlinePayment createIntent/confirmIntent). UI ConfirmationStep PaymentPlanBox; admin 'Ödeme Planı & Fiyat Gösterimi' (PaymentPlansCard).
- (4) Onay: GET /booking/{ref}/calendar.ics?email (VEVENT + 1 gün önce alarm), GET /booking/{ref}/confirmation.pdf?email (reportlab); ConfirmationStep CalendarPdfRow (.ics, Google Takvim linki, PDF).
- (5) Misafir hesabı: routes/pms/be_guest_account.py magic-link (20 dk, tek kullanım; MOCK e-postada dev_link/dev_token yanıtla döner), session 30 gün, profile (ad/telefon, geçmiş/yaklaşan rezervasyonlar + rebook_url, loyalty tier/puan), /me, /logout, /dev-verify-latest. UI templates/GuestAccountBox.js (GuestDetailsStep içinde; ?glt= ile otomatik doldurma, tekrar rezerve et çipleri).
- (6) Oda yorumları: GET /booking/room-reviews/{pid} (room_type_id | booking_ref→booking | guest_email→booking | oda adı token eşleşmesi; 4★+ 2 alıntı, verified bayrağı), PUT /booking/room-reviews/{id}/tag (admin). UI RoomReviews.js (RoomRatingBadge kartta, RoomReviewQuotes detay modalında).
- (7) Fiyat gösterimi: be_settings DEFAULTS price_display_mode (auto_by_market|tax_inclusive|tax_exclusive, enum 422), total_price_transparency, tax_exclusive_markets (liste). be_gaps string truncation [:10] → [:40] düzeltildi (BUG). BookingEngine navigator.language pazar tespiti → RatePlanRows plan-tax-excl-* / plan-total-incl-*.
- (8) Metasearch: be_distribution _ari_rows ortak veri → GET /hotel-ads/trivago|tripadvisor|bing/{pid}.xml, GET /hotel-ads/metasearch/{pid} (4 feed + konsol linkleri); DistributionCard metasearch-card.
- NOT: backend/.env STRIPE_API_KEY geçersiz (eski), STRIPE_SECRET_KEY geçerli → yeni kodda STRIPE_SECRET_KEY önceliği. Küçük: PayPage'de Stripe.js iki kez yükleniyor uyarısı (engellemez).

## Güncelleme (Faz 51 — Paket 40: Huni Uyarısı, AI Yorum Eşleme, Taksitli Ödeme, Puan Göster) — iteration_638 backend 9/10 (1 skip), admin UI %100; installments checkout seçeneği + puan satırı ekran görüntüsüyle doğrulandı
- Huni uyarısı (be_extras.py): check_funnel_alerts — son 24s adım kaybı vs önceki 7g ort. (+funnel_alert_threshold_pts, min funnel_alert_min_sessions; baz yoksa ≥%60) → admin/manager MOCK e-posta + bildirim, funnel_alerts gün/adım dedupe; funnel_alert_loop 6 saat; GET /booking/funnel-alerts/{pid}, POST .../check. UI BeFunnelCard 'Huni Uyarıları' (aç/kapa, eşik, min oturum, Şimdi kontrol et, liste). NOT: funnel event session_id ≥6 karakter (422).
- AI yorum eşleme: ai_tag_reviews (gpt-5.2 LlmChat, Emergent key; JSON review→room_type_id+confidence ≥0.5; anahtar yok/hata → kelime eşleşmesi), room_tag_attempted/source/confidence; POST /booking/room-reviews/{pid}/auto-tag, POST .../reset-tags. UI BE v2 'Oda Yorumları' sekmesi (RoomReviewsAdminCard).
- Taksit: be-settings installments_enabled/count(3)/min_amount(500); GuestDetailsStep 'N taksit × X' (inline ödeme açık + toplam ≥ min); be_payments amount_mode installments → total/n + Customer/setup_future_usage; confirm → schedule_installments (installment_1 paid + aylık scheduled, son ≤ check_in−charge_days); charge_balance(entry_idx) tek taksit çeker, run_payment_schedules taksit dalı; partial/paid geçişleri. Admin PaymentPlansCard taksit ayarları.
- Puan: be-settings loyalty_show_points/loyalty_points_per_unit(10); RatePlanRows plan-points-* (tier çarpanı sessionStorage mhb_guest_profile: silver 1.25/gold 1.5/platinum 2), ConfirmationStep confirm-points.
- Bilinen küçük: property switcher'da '<span> inside <option>' hydration uyarısı (öncesinden var, konum bulunamadı, engellemez).
